...
users = { 13115031490 : {	"username" : "Ievgenia",
					  	"level" : 1} }
...
def get_user_lvl(user_id, users):
	user = users.get(user_id)
	if not user :
		return 1
	return user.get("level")

def fill_matched_sentences(message, sentences, user_lvl)->list:
	matched_sentences = []
...
def process_message(message:str, user_lvl:int)->str:
	""" обрабтывает входящее сообщение и выдает ответ, который будет отправлен юзеру
	"""
	matched_sentences = fill_matched_sentences(message=message, user_lvl=user_lvl, sentences=sentences)
	message = create_result_message(matched_sentences=matched_sentences)
	return message
...
while True:
	...
	try:
		...
		message_user_id = last_message["message"]["from"]["id"]
	...
	if message_id > last_message_id:
		user_lvl = get_user_lvl(user_id=message_user_id, users=users)
		if not user_lvl:
			user_lvl = 1
		message_to_user = process_message(message=last_message_text,user_lvl=user_lvl)
